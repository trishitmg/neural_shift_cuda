"""CUDA shift patch for :mod:`nkd_metropolis_attn_v5`.

The v5 Metropolis model evaluates a periodic symmetric operator in two passes:
...
This patch preserves that construction while replacing Python padding/slicing
loops by the existing ``neural_shift_cuda`` primitives:

* ``shift_gather`` gathers circularly shifted guide features;
* ``accumulate_uz`` applies all forward half-plane weights together with their
  inverse-consistent reverse copies;
* a second ``shift_gather`` gathers the raw degree at the neighbouring endpoint
  of every forward edge, so all Metropolis denominators are formed in parallel;
* a second ``accumulate_uz`` applies the Metropolis-scaled symmetric kernel.

Unlike the older finite-window patch, the v5 path is fully periodic. Therefore
all wrap masks returned by ``shift_gather`` are intentionally ignored. No final
row normalization is performed; the model's unchanged ``forward`` method adds
the diagonal term ``1-d_hat`` and obtains a symmetric stochastic W.

Usage
-----

    from nkd_metropolis_attn_v5 import NKDMetropolisAttnV5
    from nkd_metropolis_attn_v5_patch import install_cuda_shift

    install_cuda_shift(NKDMetropolisAttnV5)
    model = NKDMetropolisAttnV5(...).cuda()
...
def _get_shift_tensor(self, device: torch.device) -> torch.Tensor:
    """Return cached ``(dx,dy,has_inverse)`` rows on ``device``."""
...
def _mix_heads_vec(
    ...
    """Map ``(S,B,M,H,W)`` head weights to ``(S,B,C,H,W)`` weights."""
...
# ---------------------------------------------------------------------------
# CUDA implementation of NKDMetropolisAttnV5.kernel_actions.
# ---------------------------------------------------------------------------
...
def _kernel_actions_cuda(
    ...
    """Parallel periodic application of ``K`` and ``K_hat``.

    Returns ``(K_x, degree, K_hat_x, degree_hat)`` with the same semantics as
    the reference ``kernel_actions`` method.
    """
    if x.dim() != 4:
    ...
    # ------------------------------------------------------------------
    # Compute canonical half-plane neural weights. ``shift_gather`` is
    # circular; its validity mask is deliberately ignored for periodic v5.
    # ------------------------------------------------------------------
    ...
    for start in range(0, n_shifts, chunk):
        ...
        phi_shift, _ = shift_gather(phi, shift_chunk)
    ...
    w_raw = weights.reshape(n_shifts * B, C, H, W).contiguous()

    # Pass 1: periodic symmetric raw kernel K and degree d = K e.
    K_x, degree = accumulate_uz(x.contiguous(), w_raw, shifts_t)

    # For every forward edge i -> i+delta, gather d(i+delta) circularly and
    # divide by max(d(i), d(i+delta)). The accumulator shifts this already
    # normalized forward edge to generate the reverse contribution, so the two
    # directions share exactly one pairwise denominator.
    degree_shift, _ = shift_gather(degree.contiguous(), shifts_xy)
    ...
    w_hat = w_hat.reshape(n_shifts * B, C, H, W).contiguous()

    # Pass 2: periodic symmetric K_hat and d_hat = K_hat e.
...
def install_cuda_shift(model_cls):
    ...
    The installer is idempotent. CPU tensors retain the original PyTorch
    implementation, which also serves as a numerical reference.
    """
